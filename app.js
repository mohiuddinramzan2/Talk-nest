...
window.showUserProfile = async function (uid) {
  ...
  try {
    ...
    document.getElementById("up-msg-btn").onclick   = () => {
...
async function startDirectChat(peerUser) {
  try {
    ...
    // পুরনো conversation আছে কিনা খোঁজো
    const q = query(
      collection(db, "conversations"),
      where("members", "array-contains", currentUser.uid),
      where("type", "==", "direct")
    ...
    existing.forEach(d => {
      const data = d.data();
      if (data.members.includes(peerUser.uid)) {
        convId = d.id;
      }
    });
    ...
    console.error(e);
    showToast("চ্যাট খুলতে সমস্যা: " + e.message, "error");
  } finally {
    showLoading(false);
  }
}
