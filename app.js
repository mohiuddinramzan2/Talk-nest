...
window.sendMessage = async function () {
  ...
  try {
    ...
    showToast("মেসেজ পাঠানো সম্ভব হয়নি।", "error");
  }
};
...
window.searchUsers = async function () {
  ...
  searchTimer = setTimeout(async () => {
    ...
    snap.forEach(d => {
      ...
      el.innerHTML = `<img src="${avatarUrl(u)}" alt=""/><div><div style="font-size:14px;font-weight:600">${u.name}</div><div style="font-size:12px;color:var(--accent);font-family:var(--mono)">@${u.username}</div></div><button class="start-chat-btn" onclick="showUserProfile('${u.uid}')">প্রোফাইল</button>`;
...
let _profileUser = null; // profile-এ যার তথ্য আছে সেটা globally রাখা
...
window.showUserProfile = async function (uid) {
  ...
  try {
    ...
    const u = snap.data();
    _profileUser = u; // globally save

    document.getElementById("up-avatar").src          = avatarUrl(u);
    document.getElementById("up-name").textContent    = u.name || "---";
    document.getElementById("up-uid").textContent     = "@" + (u.username || "---");
    document.getElementById("up-bio").textContent     = u.bio || "কোনো বায়ো নেই।";
    document.getElementById("up-phone").textContent   = u.phone ? "📞 +880 " + u.phone : "📞 নম্বর দেননি";
    document.getElementById("up-status").textContent  = u.online ? "🟢 অনলাইন" : "⚫ অফলাইন";
...
window.openChatFromProfile = async function () {
  if (!_profileUser) { showToast("ব্যবহারকারীর তথ্য নেই।", "error"); return; }
...
async function startDirectChat(peerUser) {
  try {
    ...
    showToast("চ্যাট শুরু করা যাচ্ছে না।", "error");
  } finally {
    showLoading(false);
  }
}
...
window.startDirectChatFromModal = async function (uid) {
  try {
    ...
    const snap = await getDoc(doc(db, "users", uid));
    const u = snap.data();
    if (!u) { showToast("ব্যবহারকারী পাওয়া যায়নি।", "error"); return; }
    await startDirectChat(u);
  } catch(e) {
